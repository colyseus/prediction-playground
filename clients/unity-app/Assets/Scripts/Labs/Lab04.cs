...
namespace Playground
{
    ...
    /// </summary>
    public class Lab04 : LabBase<Lab.BotsState>
    {
        ...
        public override void Frame(App app, double now, double dtMs)
        {
            ...
            for (int i = 0; i < _modes.Length; i++)
                if (Kb.Key(KeyCode.F1 + i)) _modes[i].Visible = !_modes[i].Visible;
        ...
        private void DrawStrip(App app)
        {
            ...
            float x0 = app.Stage.x + 24, x1 = app.Stage.xMax - 16;
            float y0 = app.Stage.yMax - h - 10, y1 = y0 + h;
            if (x1 - x0 < 200) return;

            Draw.Rect(new Rect(x0, y0, x1 - x0, h), Palette.A(Palette.Panel, 0.9f));
            ...
            _samples.ForEach((t, val) =>
            {
                ...
                Draw.Rect(new Rect(TX(t) - 1.5f, TY(val) - 1.5f, 3, 3), Palette.A(Palette.Text, 0.85f));
            });
