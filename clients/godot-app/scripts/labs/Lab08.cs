...
namespace Playground
{
    ...
    /// </summary>
    public partial class Lab08 : LabBase<Lab.GoalState>
    {
        ...
        public override void Frame(App app, double now, double dtMs)
        {
            ...
            int denyStep = Kb.Key(Key.Equal) ? 25 : Kb.Key(Key.Minus) ? -25 : 0;
            if (denyStep != 0)
            {
        ...
        {
            var v = app.View;
            var zone = new Rect2(v.SX(Sim.GoalZoneX), v.SY(Sim.GoalZoneY),
                v.S(Sim.GoalZoneW), v.S(Sim.GoalZoneH));
            Draw.Rect(zone, Palette.A(Palette.Good, 0.12f));
            Draw.Line(new Vector2(zone.Position.X, zone.Position.Y), new Vector2(zone.End.X, zone.Position.Y),
                Palette.A(Palette.Good, 0.5f), 1.5f);
            Draw.Line(new Vector2(zone.Position.X, zone.End.Y), new Vector2(zone.End.X, zone.End.Y),
                Palette.A(Palette.Good, 0.5f), 1.5f);
            Draw.Line(new Vector2(zone.Position.X, zone.Position.Y), new Vector2(zone.Position.X, zone.End.Y),
                Palette.A(Palette.Good, 0.5f), 1.5f);
            ...
            if (_banner != null && age < 1400)
            {
                ...
                Draw.Text(app.Stage.Position.X, app.Stage.Position.Y + 40, app.Stage.Size.X, _banner,
                    Palette.A(_bannerColor, a), 42, HorizontalAlignment.Center);
            }
