...
namespace Playground
{
    ...
    /// </summary>
    public partial class Lab00 : LabBase<Lab.MoveState>
    {
        ...
        public override void Render(App app)
        {
            ...
            float laneH = app.Stage.Size.Y / 2, lanePad = 22;
            _top.Fit(new Rect2(app.Stage.Position.X, app.Stage.Position.Y + lanePad,
                app.Stage.Size.X, laneH - lanePad), 26);
            _bot.Fit(new Rect2(app.Stage.Position.X, app.Stage.Position.Y + laneH,
                app.Stage.Size.X, laneH - lanePad), 26);
            ...
            float dividerY = app.Stage.Position.Y + laneH;
            float cx = app.Stage.Position.X + app.Stage.Size.X / 2;
            var rule = Palette.A(Palette.TextDim, 0.25f);
            Draw.Line(new Vector2(app.Stage.Position.X + 24, dividerY), new Vector2(cx - 90, dividerY), rule);
            Draw.Line(new Vector2(cx + 90, dividerY),
                new Vector2(app.Stage.End.X - 24, dividerY), rule);
            Draw.Text(cx - 90, dividerY - 9, 180, readout, Palette.Accent, 13, HorizontalAlignment.Center);
            ...
            Draw.Text(app.Stage.Position.X, app.Stage.End.Y - 20, app.Stage.Size.X, caption,
                Palette.A(Palette.TextDim, 0.85f), 12, HorizontalAlignment.Center);

            var h = app.Hud;
            h.Section("TELEMETRY");
            h.Row("round trip", $"{rtt:F0} ms",
                rtt > 300 ? Palette.Bad : rtt > 120 ? Palette.Warn : Palette.Good);
            double gap = new Vector2((float)px, (float)py).DistanceTo(new Vector2(me.x, me.y));
